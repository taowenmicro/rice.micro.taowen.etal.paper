...
ggsave(FileName, p1_1, width = ((1 + gnum) ), height =4*h,limitsize = FALSE)

FileName <- paste(alppath,"pd_Facet_bar", ".pdf", sep = "")
ggsave(FileName, p1_2, width = ((1 + gnum) ), height = 4*h,limitsize = FALSE)

FileName <- paste(alppath,"pd_Facet_boxbar", ".pdf", sep = "")
ggsave(FileName, p1_3, width = ((1 + gnum) ), height = 4*h,limitsize = FALSE)
...
for (method in methodlist) {
  ...
  ggsave(FileName, p3_3, width = 12, height = 7)
  FileName1 <- paste(betapath,"/a2_",method,"bray_star.jpg", sep = "")
  ggsave(FileName1 , p3_3, width = 12, height = 7)
...
barpath = paste(otupath,"/Microbial_composition_10/",sep = "")
dir.create(barpath)

phyloseq::rank_names(ps)

pst = ps %>% subset_taxa.wt("Species","Unassigned",T)


h = 2

mi=c("#E41A1C" ,"#377EB8", "#4DAF4A", "#984EA3", "#FF7F00" ,"#FFFF33",
     "#7c260b","#1B9E77", "#D95F02" ,"#7570B3" ,"#E7298A",  "#E6AB02",
     "#8DD3C7", "#FFFFB3", "#BEBADA","#FB8072" ,"#80B1D3", "#FDB462", 
     "#B3DE69","#FCCDE5","#D9D9D9", "#BC80BD","#B2DF8A" ,"#33A02C" ,
     "#FB9A99" ,"#3B4992B2","#EE0000B2","#008B45B2","#631879B2","#008280B2",
     "#BB0021B2","#5F559BB2","#A20056B2","#808180B2","#1B1919B2")
...
for (j in c("Phylum" , "Class" ,  "Order"  , "Family" , "Genus","Species")) {
  ...
  ggsave(FileName1, p4_2, width = (5+ gnum), height = 6*h )
  FileName2 <- paste(barpath,"/a2_",j,"_barflow",".jpg", sep = "")
  ggsave(FileName2, p4_2, width = (5+ gnum), height =6*h)
  
  FileName1 <- paste(barpath,"/a2_",j,"_bar",".pdf", sep = "")
  ggsave(FileName1, p4_1, width = (5+ gnum), height =6*h )
  FileName2 <- paste(barpath,"/a2_",j,"_bar",".jpg", sep = "")
  ggsave(FileName2, p4_1, width = (5+ gnum), height =6*h )
...
# library(ggClusterNet)
# library(phyloseq)
# psedger =  filter_OTU_ps(ps = ps,Top = 200)
# psedger
# res1 <- res[row.names(tax_table(psedger)),]
# result <- Plot.CompareWithCK(ps = psedger,
#                              CK = CK,
#                              j = "Genus",
#                              abun = 0.001,
#                              result = res1 )
...
if (ROC ){
  #--三种机器学习方法评测
  result = MicroRoc( ps = ps,group  = "Group",repnum = 5)
  #--提取roc曲线
  p <- result[[1]] + 
    mytheme1
  p
...
# head(tax)
# tax$Species[!tax$Species %in%c("Desulfatiglans_parachlorophenolica",
#                                          "Sulfuricella_denitrificans",
#                                          "Thiobacillus_thiophilus"
# )] = ""
...
dir.create(funcpath)
source("E:\\Shared_Folder\\Function_local\\R_function\\micro\\runRefBlast2.R")
